...
class MCPStdioServer:
    ...
    def make_server_params(self) -> StdioServerParameters:
        ...
        if env.get("OPENAPI_MCP_HEADERS") is None:
            env.update({"OPENAPI_MCP_HEADERS": os.getenv("OPENAPI_MCP_HEADERS")})

        if not list(env.values()):
            raise ValueError("Missing environment variables: OPENAPI_MCP_HEADERS")
...
class MCPInterfaceWithServer:
    ...
    async def get_tools_description(self) -> list[str]:
        ...
        logging.debug(f"Tools description :\n{tools_description}")
